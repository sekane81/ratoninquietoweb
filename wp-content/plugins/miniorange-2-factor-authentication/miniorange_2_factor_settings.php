...
* Description: This plugin provides various two-factor authentication methods as an additional layer of security for wordpress login. We Support Phone Call, SMS, Email Verification, QR Code, Push, Soft Token, Google Authenticator, Security Questions(KBA), Woocommerce front-end login, Shortcodes for custom login pages.
* Version: 3.8
* Author: miniOrange
...
class Miniorange_Authentication {
	...
	function __construct() {
		...
		$mo2f_auth_types = array('OUT OF BAND EMAIL','SMS','PHONE VERIFICATION','SOFT TOKEN','MOBILE AUTHENTICATION','PUSH NOTIFICATIONS','GOOGLE AUTHENTICATOR','KBA');
		...
		if( get_option('mo2f_activate_plugin') == 1){
			if(get_option('mo2f_login_policy')){ //password + 2nd factor enabled
				if(get_option( 'mo_2factor_admin_registration_status') == 'MO_2_FACTOR_CUSTOMER_REGISTERED_SUCCESS' ){
						...
						add_action('login_head', array( $pass2fa_login, 'miniorange_pass2login_header_field' ));
						...
						add_action( 'login_enqueue_scripts', array( $pass2fa_login,'mo_2_factor_enable_jquery') );
						add_action( 'wp_enqueue_scripts', array( $pass2fa_login,'mo_2_factor_enable_jquery') );
						...
						if(get_option('mo2f_enable_2fa_for_woocommerce') == 1){
							add_action( 'wp_head', array( $pass2fa_login, 'miniorange_pass2login_header_field' ));
	...
	function miniorange_auth_menu() {
		...
		$miniorange_role = $current_user->roles[0];
		if( !current_user_can( 'manage_options' ) && get_option('mo2fa_'.$miniorange_role) && get_option( 'mo_2factor_admin_registration_status') == 'MO_2_FACTOR_CUSTOMER_REGISTERED_SUCCESS' && get_option( 'mo2f_miniorange_admin') != $current_user->ID && get_option('mo2f_activate_plugin') == 1){
			$user_register = new Miniorange_User_Register();
				$page = add_menu_page ('miniOrange 2 Factor Auth', get_option('mo2f_custom_plugin_name') , 'read', 'miniOrange_2_factor_settings', array( $user_register, 'mo2f_register_user'), $iconurl);
		}else if(current_user_can( 'manage_options' )){
				$page = add_menu_page ('miniOrange 2 Factor Auth',  get_option('mo2f_custom_plugin_name'), 'manage_options', 'miniOrange_2_factor_settings', array( $this, 'mo_auth_login_options' ),$iconurl);
		}
	}
	...
	function mo_2_factor_enable_frontend_style() {
		wp_enqueue_style( 'mo2f_frontend_login_style', plugins_url('includes/css/front_end_login.css?version=3.4', __FILE__));
		wp_enqueue_style( 'bootstrap_style', plugins_url('includes/css/bootstrap.min.css?version=3.4', __FILE__));
	}
	
	function plugin_settings_style() {
		wp_enqueue_style( 'mo_2_factor_admin_settings_style', plugins_url('includes/css/style_settings.css?version=3.4', __FILE__));
					...
					if(json_last_error() == JSON_ERROR_NONE) { 
							if($response['status'] == 'ERROR'){
								...
								if($selectedMethod == 'OUT OF BAND EMAIL'){
									...
									$selectedMethod = "QR Code Authentication";
								}else if(get_user_meta( $current_user->ID,'mo2f_selected_2factor_method',true) == 'SMS'){
									$authType = "OTP Over SMS";
								}
								update_user_meta($current_user->ID,'mo_2factor_user_registration_status','MO_2_FACTOR_PLUGIN_SETTINGS');
								delete_user_meta($current_user->ID,'mo2f_configure_test_option');
